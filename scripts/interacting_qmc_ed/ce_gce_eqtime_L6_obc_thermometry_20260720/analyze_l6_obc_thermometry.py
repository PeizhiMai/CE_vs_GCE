#!/usr/bin/env python3
"""Build the L=6 OBC equal-time comparison, mismatch, and thermometry assets.
...
STATUS_COLORS = {
    ...
    "incomplete": "#E8EBEF", "no_gce_curve": "#E8EBEF",
}
...
def save_figure(fig: plt.Figure, stem: Path, multipage: PdfPages | None = None) -> None:
    ...
    # 160 dpi on a 24-inch scientific canvas is already approximately 4K.
    # Keeping the raster bounded avoids retaining hundreds of megabytes while
    # the 14-page vector PDF is open.
...
def comparison_figure(data: pd.DataFrame, status: pd.DataFrame, quantity: str, label: str) -> plt.Figure:
    ...
    fig, axes = plt.subplots(5, 4, figsize=(24.0, 10.5), sharex=True)
    ...
    fig.suptitle(f"L=6 OBC — {label}: CE versus GCE at actual T=1/β", fontsize=18, fontweight="bold", y=.995)
...
def thermometer_figure(observed: pd.DataFrame, status: pd.DataFrame, quantity: str, label: str) -> plt.Figure:
    ...
    fig, axes = plt.subplots(5, 4, figsize=(24.0, 10.5), sharex=True, sharey=False)
    for ri, u in enumerate(US):
        for ci, n in enumerate(NS):
            ax = axes[ri, ci]
    ...
    fig.suptitle(f"L=6 OBC {label} thermometry — lines join actual unique-temperature points only", fontsize=18, fontweight="bold", y=.998)
...
def scorecard_figure(summary: pd.DataFrame) -> plt.Figure:
    ...
    ax.set_title("L=6 OBC thermometry scorecard", fontsize=18, fontweight="bold")
    table_ax.axis("off")
    cells = []
...
def workflow_scorecard(data: pd.DataFrame, status: pd.DataFrame) -> pd.DataFrame:
    """Return one strict, presentation-ready OBC completion/provenance row."""
    qmc_status = status[~np.isclose(status.U, 0.0)].copy()
    ce_qmc = qmc_status[qmc_status.ensemble.eq("CE")]
    gce_qmc = qmc_status[qmc_status.ensemble.eq("GCE")]
    pilots = ce_qmc[(ce_qmc.U > 0) & ce_qmc.beta.isin([5.0, 6.7, 10.0])]
    gce_final = data[(data.ensemble == "GCE") & (~np.isclose(data.U, 0.0))].copy()
    density_error = np.abs(
        pd.to_numeric(gce_final["N_mean"], errors="coerce")
        - pd.to_numeric(gce_final["Ntot"], errors="coerce")
    )
    mu_shift = np.abs(
        pd.to_numeric(gce_final["mu"], errors="coerce")
        - pd.to_numeric(gce_final["mu_L6_PBC_reference"], errors="coerce")
    )
    positive_ce = data[(data.ensemble == "CE") & (data.U > 0) & (data.beta >= 5)]
    phase = pd.to_numeric(positive_ce.get("average_phase_abs", pd.Series(dtype=float)), errors="coerce")
    commits = sorted({str(x) for x in data.get("project_commit", pd.Series(dtype=str)).dropna() if str(x)})
    smoq_commits = sorted({str(x) for x in data.get("smoqydqmc_commit", pd.Series(dtype=str)).dropna() if str(x)})
    smoq_versions = sorted({str(x) for x in data.get("smoqydqmc_version", pd.Series(dtype=str)).dropna() if str(x)})
    return pd.DataFrame([{
        "boundary": "open",
        "conditions": 192,
        "ensemble_rows": int(len(status)),
        "exact_u0_conditions": 40,
        "exact_u0_ensemble_rows": int((np.isclose(status.U, 0.0) & status.status.eq("exact_final")).sum()),
        "interacting_ce_rows": int(len(ce_qmc)),
        "interacting_ce_strict_final": int(ce_qmc.status.eq("strict_final").sum()),
        "interacting_ce_sign_limited": int(ce_qmc.status.eq("sign_limited").sum()),
        "positive_high_beta_pilots": int(len(pilots)),
        "positive_high_beta_admitted": int(pilots.status.eq("strict_final").sum()),
        "positive_high_beta_sign_limited": int(pilots.status.eq("sign_limited").sum()),
        "interacting_gce_rows": int(len(gce_qmc)),
        "interacting_gce_strict_final": int(gce_qmc.status.eq("strict_final").sum()),
        "gce_density_within_0p03": int((density_error <= 0.03 + 1e-12).sum()),
        "gce_density_median_abs_error": float(density_error.median()) if density_error.notna().any() else np.nan,
        "gce_density_max_abs_error": float(density_error.max()) if density_error.notna().any() else np.nan,
        "gce_pbc_seed_provenance_rows": int(gce_final["mu_L6_PBC_reference"].notna().sum()),
        "gce_l8_inherited_provenance_rows": int(gce_final["mu_L8_reference"].notna().sum()),
        "gce_mu_shift_from_pbc_median_abs": float(mu_shift.median()) if mu_shift.notna().any() else np.nan,
        "gce_mu_shift_from_pbc_max_abs": float(mu_shift.max()) if mu_shift.notna().any() else np.nan,
        "positive_high_beta_phase_median_abs": float(phase.median()) if phase.notna().any() else np.nan,
        "positive_high_beta_phase_min_abs": float(phase.min()) if phase.notna().any() else np.nan,
        "site_count": 36,
        "nn_bond_count": 60,
        "nnn_bond_count": 50,
        "project_commit": ";".join(commits),
        "smoqydqmc_version": ";".join(smoq_versions),
        "smoqydqmc_commit": ";".join(smoq_commits),
    }])
...
def main() -> None:
    ...
    if not data.empty:
        if not data["boundary"].astype(str).eq("open").all():
            raise SystemExit("refusing mixed/PBC data: every snapshot row must have boundary=open")
        for col, expected in (("site_count", 36), ("nn_bond_count", 60), ("nnn_bond_count", 50)):
            values = pd.to_numeric(data[col], errors="coerce")
            if not values.eq(expected).all():
                raise SystemExit(f"invalid OBC geometry metadata in {col}; expected {expected}")
    ...
    write_table(args.outdir / "data" / "l6_obc_workflow_scorecard.tsv", workflow_scorecard(data, status))

    assets: list[dict[str, object]] = []
    figures = args.outdir / "figures"
    with PdfPages(args.outdir / "L6_OBC_equal_time_thermometry_14slide_assets.pdf") as pdf:
        order = 1
        for quantity, label in PRIMARY:
            ...
            assets.append({"order": order, "kind": "primary_comparison", "quantity": quantity,
                           "title": f"L=6 OBC — {label}: CE versus GCE", "png": str(stem.with_suffix('.png')), "pdf": str(stem.with_suffix('.pdf'))})
            order += 1
        for quantity, label in NNN:
            ...
            order += 1
        mismatch_specs = (
            ("delta_CE_minus_GCE", "L=6 OBC — Absolute CE−GCE mismatch versus T", "O_CE−O_GCE", "absolute"),
            ("relative_CE_minus_GCE_over_abs_GCE", "L=6 OBC — Fractional CE−GCE mismatch versus T", "(O_CE−O_GCE)/|O_GCE|", "relative"),
            ("gce_range_normalized_CE_minus_GCE", "L=6 OBC — GCE-range-normalized mismatch versus T", "ΔO/range_T(O_GCE)", "range"),
        )
        ...
        assets.append({"order": order, "kind": "thermometry_scorecard", "quantity": "all",
                       "title": "L=6 OBC thermometry scorecard", "png": str(stem.with_suffix('.png')), "pdf": str(stem.with_suffix('.pdf'))})
        order += 1
        for quantity, label in PRIMARY:
            ...
            assets.append({"order": order, "kind": "thermometer", "quantity": quantity,
                           "title": f"L=6 OBC — {label} thermometry", "png": str(stem.with_suffix('.png')), "pdf": str(stem.with_suffix('.pdf'))})
            order += 1
    if order != 15:
