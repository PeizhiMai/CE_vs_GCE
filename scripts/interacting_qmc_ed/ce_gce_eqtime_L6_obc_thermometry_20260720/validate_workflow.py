...
def main()->None:
    ...
    tuning_rule=(HERE/"MU_TUNING_RULE.md").read_text()
    check("ce_gce_eqtime_L6_thermometry_20260719/MU_TUNING_RULE.md" in tuning_rule,"OBC mu tuning cites the authoritative PBC rule")
    tuner=(HERE/"tune_mu.py").read_text()
    check("dynamic_center_" in tuner and "bracket_refinement_required" in tuner and "--max-final-bracket-width" in tuner,"mu tuning uses directed dynamic centers and tight-bracket refinement")
    check("symmetric_extension_pm" not in tuner,"mu tuning never launches mirrored symmetry probes")
    production_runner=(HERE/"run_gce_production_manifest_task.sh").read_text()
    check("final OBC mu bracket width" in production_runner and "is outside final bracket" in production_runner,"production runner fails closed on final mu bracket")
    repair=(HERE/"repair_stale_l6_roots.py").read_text()
    check("superseded_bad_final_bracket" in repair,"repair excludes superseded bad-bracket production")
    audit=(HERE/"audit_status.py").read_text();collector=(HERE/"collect_l6_results.py").read_text()
    check("superseded_bad_bracket" in audit and "validate_final_mu_bracket" in collector,"audit and collection enforce final mu brackets")
