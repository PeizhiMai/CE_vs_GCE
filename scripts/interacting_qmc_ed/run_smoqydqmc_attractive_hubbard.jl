# # Attractive square Hubbard Model with SmoQyDQMC

# In this example we will work through simulating the attractive Hubbard model on a square lattice.
...
function run_simulation(;
    ...
    filepath = joinpath(@__DIR__, "..", "..", "results", "interacting_qmc_ed", "smoqydqmc_attractive_hubbard") # Filepath to where data folder will be created.
)
...
# in these cases.
    ...
    U < 0 || throw(ArgumentError("This driver is for the attractive Hubbard model: pass a negative U, e.g. -4.0"))
...
# [`HubbardSpinHirschHST`](@ref) and [`HubbardSpinGaussHermiteHST`]. In this attractive-Hubbard driver we use the
# [`HubbardDensityHirschHST`](@ref) type because it is real for U <= 0.

    ## Apply Density Hirsch Hubbard-Stratonovich (HS) transformation to decouple the Hubbard interaction,
...
# which were passed as arguments to the `run_simulation` function.
    ...
    ## is [bond_px, bond_py, bond_pxpy, bond_pxny]. SmoQyDQMC exports both equal-time and
    ## imaginary-time-integrated current correlations in momentum space. The integrated
    ## current(q) values at q=(2π/L,0) and q=(0,2π/L) are used to estimate
...
# [`HubbardDensityHirschHST`](@ref) type used in this driver is real for the attractive case ``U \le 0``
# and complex for ``U > 0``. Therefore, this attractive-Hubbard driver sets
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    run_simulation(;
        ...
        ph_sym_form = length(ARGS) >= 11 ? parse(Bool, ARGS[11]) : true,
        filepath = length(ARGS) >= 12 ? ARGS[12] : joinpath(@__DIR__, "..", "..", "results", "interacting_qmc_ed", "smoqydqmc_attractive_hubbard"),
        Ly = length(ARGS) >= 13 ? parse(Int, ARGS[13]) : parse(Int, ARGS[5])
    )
end

# For instance, the command
# ```
# > ./scripts/run_julia_local.sh scripts/interacting_qmc_ed/run_smoqydqmc_attractive_hubbard.jl 1 -4.0 0.0 0.0 4 2.0 2000 2000 40 5
# ```
# runs a DQMC simulation of a ``N = 4 \times 4`` attractive square Hubbard model at inverse temperature ``\beta = 2.0``
# with interaction strength ``U = -4.0,`` chemical potential ``\mu = 0.0`` and next-nearest-neighbor hopping amplitude ``t^\prime = 0.0``.
