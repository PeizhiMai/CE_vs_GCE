...
function write_jld2_checkpoint_mpi_serial(
    ...
    # JLD2/HDF5 writes from many Julia MPI ranks at the same time can be
    # fragile on shared filesystems.  Serialize checkpoint writes across ranks
    # while still writing one checkpoint file per rank.
    for write_pID in 0:(comm_size-1)
        MPI.Barrier(comm)
        if pID == write_pID
            local_checkpoint_timestamp = write_jld2_checkpoint(
                simulation_info;
    ...
    if (local_checkpoint_timestamp + checkpoint_freq - start_timestamp) ≥ runtime_limit
        MPI.Barrier(comm)
        MPI.Finalize()
        exit(error_code)
    end
...
function run_simulation(
    ...
    measurement_profile = "full", # "full" production measurements, "density-only" for chemical-potential tuning, or autocorrelation probes.
...
# Second, we need to convert the `checkpoint_freq` and `runtime_limit` from hours to seconds.
    ...
    measurement_profile in ("full", "density-only", "autocorr-current", "autocorr-global") ||
        throw(ArgumentError("measurement_profile must be one of: full, density-only, autocorr-current, autocorr-global"))
