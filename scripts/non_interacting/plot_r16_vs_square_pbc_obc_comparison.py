...
SQUARE_PBC_ROOT = ROOT / "results" / "non_interacting" / "spinless_square_pbc_comparison_by_L"
SQUARE_OBC_ROOT = ROOT / "results" / "non_interacting" / "spinless_square_obc_comparison_by_L"
...
def disk_data(radius: int) -> dict[str, np.ndarray | int]:
    disk_dir = ROOT / "results" / "non_interacting" / "spinless_disk_comparison_by_radius" / f"radius_{radius}"
...
def write_metadata(
    ...
    lines = [
        f'comparison = "Disk OBC R={disk_radius} versus square L={square_L} OBC/PBC"',
        ...
        'curves = "disk_gce, disk_ce, square_obc_gce, square_obc_ce, square_pbc_gce, square_pbc_ce"',
        ...
        f'data_sources = "results/non_interacting/spinless_disk_comparison_by_radius/radius_{disk_radius}, results/non_interacting/spinless_square_obc_comparison_by_L/L_{square_L}, results/non_interacting/spinless_square_pbc_comparison_by_L/L_{square_L}"',
    ]
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(lines) + "\n")
...
def main() -> None:
    ...
    title = (
        f"Disk OBC: R={args.disk_radius}, V={disk['nsites']}   |   "
        f"Square OBC/PBC: L={args.square_L}, V={pbc['nsites']}"
    )
