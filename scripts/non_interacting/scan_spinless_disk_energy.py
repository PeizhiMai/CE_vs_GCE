...
DEFAULT_OUT_DIR = ROOT / "results" / "non_interacting" / "spinless_disk_comparison_by_radius"
...
def canonical_energy_per_particle(beta: float, energies: np.ndarray, target_n: int) -> float:
    ...
    prob = np.zeros(target_n + 1, dtype=float)
    energy_sum = np.zeros(target_n + 1, dtype=float)
    prob[0] = 1.0

    for i, (p, ε) in enumerate(zip(probs, energies), start=1):
        q = 1.0 - p
        max_n = min(i, target_n)
        old_prob_n = prob[1 : max_n + 1].copy()
        old_energy_n = energy_sum[1 : max_n + 1].copy()
        old_prob_nm1 = prob[:max_n].copy()
        old_energy_nm1 = energy_sum[:max_n].copy()
        prob[1 : max_n + 1] = q * old_prob_n + p * old_prob_nm1
        energy_sum[1 : max_n + 1] = q * old_energy_n + p * (
            old_energy_nm1 + ε * old_prob_nm1
        )
        prob[0] *= q
        energy_sum[0] *= q

    prob_total = prob[target_n]
    if prob_total <= 0.0:
        raise RuntimeError("Canonical conditioning probability underflowed to zero.")

    energy = energy_sum[target_n] / prob_total
...
def make_radius_plot(
    ...
    note = f"R={int(radius) if float(radius).is_integer() else radius}, V={nsites}, N={canonical_n}, GCE tuned to <N>={canonical_n}"
...
def cached_spectrum_path(out_dir: Path, radius: float) -> Path:
    return out_dir / format_radius_dir(radius) / "single_particle_energies.tsv"


def load_or_compute_disk_spectrum(radius: float, out_dir: Path) -> tuple[np.ndarray, int]:
    spectrum_path = cached_spectrum_path(out_dir, radius)
    if spectrum_path.exists():
        energies = np.loadtxt(spectrum_path, delimiter="\t", skiprows=1, usecols=1)
        energies = np.atleast_1d(np.asarray(energies, dtype=float))
        return energies, int(energies.size)
    ...
    write_tsv(
        ...
        [[idx + 1, f"{value:.12f}"] for idx, value in enumerate(energies)],
    )
    return energies, len(system.sites)
