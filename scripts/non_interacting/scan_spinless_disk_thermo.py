...
def canonical_sector_data(
    ...
    for idx, energy in enumerate(energies):
        ...
        old_prob_n = prob[1 : upper + 1].copy()
        old_energy_n = energy_sum[1 : upper + 1].copy()
        old_prob_nm1 = prob[:upper].copy()
        old_energy_nm1 = energy_sum[:upper].copy()
        prob[1 : upper + 1] = q * old_prob_n + p * old_prob_nm1
        energy_sum[1 : upper + 1] = q * old_energy_n + p * (
            old_energy_nm1 + energy * old_prob_nm1
        )
        prob[0] *= q
        energy_sum[0] *= q
