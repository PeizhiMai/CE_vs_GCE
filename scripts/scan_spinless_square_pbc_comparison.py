...
DEFAULT_OUT_DIR = ROOT / "results" / "spinless_square_pbc_comparison_by_L"


def square_pbc_energies(L: int, t_hop: float = 1.0) -> np.ndarray:
...
def make_plot(
    ...
    ax.set_title(f"{title_prefix}, L={L}", fontsize=24)
...
def write_metadata(path: Path, L: int, nsites: int, canonical_n: int, beta_grid: list[float]) -> None:
    lines = [
...
def solve_L(L: int, beta_grid: list[float], out_dir: Path, t_hop: float = 1.0) -> dict[str, float]:
    energies = square_pbc_energies(L, t_hop=t_hop)
    nsites = L * L
    canonical_n = nsites // 2
    ...
    for idx, beta in enumerate(beta_grid):
        ...
        ce_data = canonical_sector_data(beta, energies, sectors, mu_ref=0.0)
    ...
    l_dir = out_dir / f"L_{L}"
    write_tsv(
        l_dir / "square_pbc_thermo_vs_beta.tsv",
        [
            "beta",
            "temperature",
            "nsites",
    ...
    write_tsv(
        l_dir / "square_pbc_specific_heat_vs_temperature.tsv",
        [
            "temperature",
    ...
    write_metadata(l_dir / "metadata.toml", L, nsites, canonical_n, beta_grid)
    make_plot(
        ...
        title_prefix="Spinless Square PBC Energy vs T",
    )
    make_plot(
        ...
        title_prefix="Spinless Square PBC Specific Heat vs T",
    )
    make_plot(
        ...
        title_prefix="Spinless Square PBC Compressibility vs T",
    )
    make_plot(
        ...
        title_prefix="Spinless Square PBC Entropy vs T",
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Exact CE vs GCE comparison for a spinless periodic LxL square lattice "
            "at half-filling."
        )
    )
    parser.add_argument(
...
def main() -> None:
    ...
    for L in Ls:
        result = solve_L(L, beta_grid, out_dir, t_hop=args.t)
        summary_rows.append(
            [
                L,
                int(result["nsites"]),
    ...
    print("Computed spinless square-lattice PBC comparison")
